% 1. Read microphone signal (S+N)
[mic_signal, fs_mic] = audioread("D:\STMicroelectronics\Matlab_Main\Code\Simulation Sound\simulated_sound.wav");
...
figure;
subplot(3,1,1);
plot(time_mic, mic_signal); title('Microphone Signal (S+N)');
subplot(3,1,2);
plot(time_mic, anti_noise_resampled); title('IMU Anti-Noise Signal (N*)');
subplot(3,1,3);
plot(time_mic, error_signal); title('Error Signal (~S)');
